...
$TEMP_FOLDER    = "/home/webpages/lima-city/bipulse-kreativwerkstatt/ftp/import/hoya/";
$ARCH_FOLDER    = "/home/webpages/lima-city/bipulse-kreativwerkstatt/ftp/import/archive/hoya/";
$MAILS_FOLDER   = "/home/webpages/lima-city/bipulse-kreativwerkstatt/ftp/import/archive/html/hoya/";
...
/* ------------------------ Load CSV file into table ------------------------ */
    ...
    $message = sprintf("%s\n", mysqli_info($conn));
    write_mysql_log($message, '1', NULL, $conn);
...
write_mysql_log($message, '2', NULL,$conn);
} 
...
where customeremail not in (SELECT distinct(customeremail) FROM unsubscribe) and customernumber != 0 ";

if($_GET['cid']){
    $sql .= "and customernumber = '".$_GET['cid']."' ";
}
$sql .= "GROUP BY customernumber ORDER BY customernumber ASC";
$result = mysqli_query($conn, $sql);
...
/* -------------------------- Get customer language ------------------------- */
    ...
    WHERE customernumber = ".$customernumber." order by orderplanneddate";
...
#print_r($blocks);
  ...
  #  if($_GET['mail']){
       ...
       # $mail->addAddress('allaya@hoya.com', 'Yasmine Allaya');
        ...
        if (!$mail->send()) {
            write_mysql_log($mail->ErrorInfo, NULL, '2', $conn);
            #echo 'Mailer Error: ' . $mail->ErrorInfo;
        } else {
            write_mysql_log('Notification sent to customernumer '.str_pad($row['customernumber'], 5, '0', STR_PAD_LEFT)." (".$row['customeremail'].').', $customernumber, '2', $conn);
        }
   ...
   # }
    ...
    #echo $scriptpath.$folderName;
    if(!is_dir($folderName))
		{
			mkdir($scriptpath.$folderName, 0777, true);
		}

    
    $myFile = $scriptpath.$folderName."/".$row['customernumber'].".html"; // or .php   
    $fp = fopen($myFile,'w+');
    fwrite($fp, utf8_encode($mailbody));

    write_mysql_log('View sent email to customer '.$row['customernumber'].': <a href="/assets/snippets/phpimport/'.$folderName.'/'.$row['customernumber'].'.html" target="_blank">E-mail</a>', $customernumber, '3', $conn);
    fclose($fp);
