...
curl_setopt_array($curl, array(
  ...
  CURLOPT_HTTPHEADER => array(
	...
	$ordconf = ($order['stationId'] != 14 || $order['stationId'] != 15) ? "Y" : "N";
		...
		VALUES ('".$store['Account']['Hoya_Account_ID__c']."','".$store['AccountId']."', '".$store['Account']['Name']."', '$customername2', '".$store['Account']['Shop_Street__c']."', '".$store['Account']['Shop_Postal_Code__c']."', '".$store['Account']['Shop_City__c']."', '$customercountry', 'nl-nl', '$customeremail', '', '', '".$ordconf."', '', '".$orderDate."', '".$order["reference1"]."', '".$order["reference2"]."', '".$deliveryDate."', '".$orderDelay."', '', '', '', '', '".$orderLensName."', '".$orderNumber."', '".$tt_station_id."', '".$hoyailog_station_code."');\n";
...
//echo $output;
 $sql = "TRUNCATE TABLE `hoya_daily_data_".$countrycode."`;";
